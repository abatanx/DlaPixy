...
- 既定キャンバスサイズ: `256x256`
- キャンバスの幅 / 高さは独立して扱い、編集・選択範囲・floating 貼り付け・プレビュー・slice・保存 / 再読込まで非正方形キャンバスを扱える
- グリッドはキャンバス解像度ではなくオーバーレイ間隔 (`0..max(width,height)`, `0` はなし)
- 初期表示倍率は `1x`
- 高速ドラッグでも補間して描画欠けを防ぐ
- Undo 対応
...
- floating 貼り付け / 移動では以下を扱える
  - ドラッグ移動
  - 8 ハンドルでリサイズ
  - `きっちり / αなめらか` の固定比率リサイズ
  - `置換 / ブレンド` のプレビュー切替
  - `Enter` で確定、`Esc` でキャンセル
- floating のドラッグ / リサイズ中は、一時 preview buffer でキャンバスだけを追従させ、overlay ラベルはそのまま追従しつつ、確定 `pixels`・palette usage・重い preview は操作終了 / 確定まで遅延する
- 選択済みピクセルをそのまま floating として移動できる
- 通常選択状態では、点線の辺ドラッグで選択枠だけを移動でき、8 ハンドルで調整でき、上下左右キーで `1px` ずつ移動でき、矩形ドラッグ中に `Shift` を押している間は正方形に固定され、ドラッグ可能な辺やハンドル上ではカーソルも move / resize に切り替わる
- `Q` / `V` は選択系 mode として扱い、通常選択では `Q` が active、floating では `V` が active になり、floating 中に `Q` を押すと通常選択へ確定して戻れる
- floating 中に `W` / `E` / `P` を押した場合は、現在位置で確定して選択を残したまま、描画 / 消しゴム / 塗りつぶしへ切り替える
- floating のリサイズは通常は自由変形で、ハンドルドラッグ中に `Shift` を押している間だけ元画像のアスペクト比を維持し、離すと同じドラッグ中でも自由変形へ戻る
- floating 矩形の外をクリックしたときは、新しい選択を始めず、floating を確定して選択自体も解除する
- 回転モーダルでは循環移動、正方形限定の `90deg` 回転、水平 / 垂直フリップが使える
- 描画系ツールは選択範囲内だけに制限される
- 選択系の共通基盤として renderer 側コンテキストメニュー基盤を追加し、現在の選択範囲を右クリックすると文脈付きメニューを開ける。通常選択では `Copy` / `Delete` / `Floating` / 回転 / Preview 追加、floating では確定 / キャンセル / 合成モード / 拡縮モードを出し、メニュー外クリック / `Esc` で閉じる
...
- ネイティブ `Palette` メニューから不要パレット削除モーダルを開けて、未使用色のうち `locked` や `caption` 付きも含めるか選んで一括削除でき、操作は Undo できる
- ネイティブ `Palette` メニューとサイドバーからテキスト追加モーダルを開けて、受け入れ可能な16進色コードを任意テキストから抽出し、プレビューしながら新規色だけを共通 `caption` / `lock` 付きでまとめて追加できる
...
- PNG 読込では非正方形サイズを維持し、エディタの最小 / 最大制限で拡大縮小が必要なときも、可能な範囲で縦横比を維持する
...
- renderer 側コンテキストメニューは、今後の対象別メニュー追加を見据えて「状態 + 項目定義」の再利用前提で組んでいる。
